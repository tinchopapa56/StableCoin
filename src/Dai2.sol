...
contract Dai2 is ERC20Burnable, Ownable {
    error Dai2__err_MustBeMoreThanZero();
    error Dai2__err_BurnAmountExceedsBalance():
    error Dai2__err_NotZeroAddress():

    constructor() ERC20("Dai2", "DSC") {}

    function burn(uint256 _amount) public override onlyOwner{
        uint256 balance = balanceOf(msg.sender);
        if(_amount <= 0) {
            revert Dai2__err_MustBeMoreThanZero();
        }
        if(balance < _amount) {
            revert Dai2__err_MustBeMoreThanZero();
        }
    ...
    function mint(address _to, uint256 _amount) external onlyOwner returns(bool) {
        if(_to == address(0)){
            revert Dai2__err_NotZeroAddress
        }
        if(_amount <= 0) {
            revert Dai2__err_MustBeMoreThanZero();
        }
